...
export default function PhoneLoginModal({
  ...
  const handleSelectPreset = (username: string, pass: string) => {
    setMode('login');
    setLoginIdentifier(username);
    setLoginPassword(pass);
    setErrorMsg(null);
    setSuccessMsg(`Selected ${username}. Click "Log In" to proceed.`);
  };
  ...
  return (
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-3 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in">
      <div className="relative w-full max-w-lg bg-slate-900 border border-slate-700/80 rounded-3xl p-5 sm:p-6 shadow-2xl text-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div>
              <div className="font-bold text-white flex items-center gap-1.5">
                ...
                <span className="font-mono text-emerald-400 font-black">
                  {totalRegistered} Active Accounts
                </span>
              </div>
              <p className="text-[10px] text-slate-400">
                Official BFP Madrid Municipal Emergency System
              </p>
            </div>
          </div>
        ...
        {/* Predefined Accounts Quick Bar */}
        <div className="mt-4">
          <div className="text-[11px] font-bold uppercase text-slate-400 mb-2 flex items-center justify-between">
            <span>Official Station Admins (Duty Dispatchers)</span>
            <span className="text-[10px] text-amber-400 font-mono">Password: admin</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              type="button"
              onClick={() => handleSelectPreset('Admin1', 'admin')}
              className={`p-2.5 rounded-xl border text-left transition flex flex-col justify-between ${
                loginIdentifier.toLowerCase() === 'admin1'
            ...
            >
              ...
              <div className="text-[10px] text-slate-300 font-semibold mt-1">BFP Station Commander</div>
              <div className="text-[9px] text-slate-500">Official Admin 1</div>
            </button>

            {/* Admin 2 Button */}
            <button
              type="button"
              onClick={() => handleSelectPreset('Admin2', 'admin')}
              className={`p-2.5 rounded-xl border text-left transition flex flex-col justify-between ${
                loginIdentifier.toLowerCase() === 'admin2'
                  ? 'bg-amber-950/70 border-amber-500 shadow-md'
                  : 'bg-slate-950/60 border-slate-800 hover:border-slate-700'
              }`}
            >
              ...
              <div className="text-[10px] text-slate-300 font-semibold mt-1">BFP Operations Chief</div>
              <div className="text-[9px] text-slate-500">Official Admin 2</div>
            </button>
          </div>
        </div>
        ...
        <div className="mt-4 flex border-b border-slate-800">
          ...
          >
            ...
            <span>Sign In (Admin / Citizen)</span>
          </button>
          <button
            onClick={() => {
              setMode('register');
        ...
        {mode === 'login' ? (
          ...
          <form onSubmit={handleLoginSubmit} className="mt-4 space-y-3.5">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g. Admin1, Admin2, or your registered mobile..."
            ...
            <div>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 mt-1">
                <span>Station Dispatcher Passwords: Admin1 / Admin2 &rarr; admin</span>
              </div>
            </div>
          ...
          /* Registration Form for 297 Open Accounts */
          <form onSubmit={handleRegisterSubmit} className="mt-4 space-y-3">
            <div className="p-2.5 rounded-xl bg-emerald-950/40 border border-emerald-800/80 text-[11px] text-emerald-300">
              Registration is required for all residents of Madrid, Surigao del Sur to report incidents, track responder GPS, and receive direct BFP dispatch updates.
            </div>
